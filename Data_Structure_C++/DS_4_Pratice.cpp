...
int main()
{
	...
	priority_queue<Card, vector<Card>, greater<Card> > cardplayer;
	...
	for (i = 0; i < players; i++) { //分牌
		for (j = 0; j < cardno; j++) {
			m = cano[cardno * i + j];
			cardplayer.push(allcards[m]);
		}
		cards_players.push_back(cardplayer);
		while (!cardplayer.empty()) cardplayer.pop();
	}
